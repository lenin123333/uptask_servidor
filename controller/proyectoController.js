...
const obtenerProyectos= async (req,res)=>{
    //Poer defecto apunt and asiq ue por eso es el or
...
const obtenerProyecto= async (req,res)=>{
    const {id} = req.params
    const valido= mongosse.Types.ObjectId.isValid(id)
    if (!valido){
...
const eliminarColaborador= async (req,res)=>{
    ...
    const usuario = await Usuario.findOne({email:req.usuario.email}).select('-confirmado -createdAt -password -token -updatedAt -__v')
    // El colaborador no es el admin del proyecto
    if(proyecto.creador.toString() !== usuario._id.toString()){
